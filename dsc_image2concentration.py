...
def main(iFname, maskFname, paramFilePath, physioLogFolder, oFname, r2GdInBlood):
    """Main.
    ...
    # Physio processing
    # ----------------------------------------------------------------------------------------------------------------------
    subjID = os.path.abspath(iFname).split('/')[-4]
    baseDir = '/'.join(os.path.abspath(iFname).split('/')[0:-4])
...
if __name__ == "__main__":
    ...
    requiredArgs.add_argument('-physio', dest='physioLogFolder', help='Path to folder including the physiologs.', type=str, required=True)
    requiredArgs.add_argument('-o', dest='oFname', help='Filename for output image and mat file.', type=str, required=True)
    # requiredArgs.add_argument('-inj', dest='injRep', help='Repetition number of contrast agent injection.', type=int, required=True)
    ...
    #                                                             ' format <FA map voltage>,<DSC voltage>,<DSC flip angle>', type=str, required=False, default='0,0,0')
    ...
    # run main
    main(iFname=args.iFname, maskFname=args.maskFname, paramFilePath=args.paramFilePath, physioLogFolder=args.physioLogFolder, oFname=args.oFname, r2GdInBlood=args.r2GdInBlood)
